...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_normal = (x - m) / np.sqrt(v + eps)
        out = gamma * x_normal + beta
        cache = (x_normal, gamma, beta, m, v, x, eps)
...
def batchnorm_backward(dout, cache):
    ...
    x_norm, gamma, beta, m, v, x, eps = cache
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(x_norm * dout, axis=0)
    N = x.shape[0]
    dx_norm = dout * gamma
    d1 = dx_norm / (np.sqrt(v + eps))
    d1_sqrt_v = np.sum(dx_norm * (x - m), axis=0)
    dsqrt_v = - d1_sqrt_v / (v + eps)
    dv = 0.5 * dsqrt_v / np.sqrt(v + eps)
    dx_square = np.ones_like(x) / N * dv
    d2 = dx_square * (x - m) * 2
    dm = d1 + d2
    dx = dm + np.ones_like(x) / N * np.sum(-dm, axis=0)
...
def batchnorm_backward_alt(dout, cache):
    ...
    x_normal, gamma, beta, m, v, x, eps = cache
    N = x.shape[0]
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(x_normal * dout, axis=0)
    dvar = (dout * gamma * (x - m) * (-0.5) * np.power(v + eps, -1.5)).sum(axis = 0)
    dmean = np.sum((dout * gamma) * (-1) * np.power(v + eps, -0.5), axis = 0)
    dmean += dvar * np.sum(-2 * (x - m), axis = 0) / N
    dx = dout * gamma * np.power(v + eps, -0.5) + dvar*2*(x - m) / N + dmean / N
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    v = np.var(x_T, axis=0)
    x_normal_T = (x_T - m) / np.sqrt(v + eps)
    x_normal = x_normal_T.T
...
def layernorm_backward(dout, cache):
    ...
    dv = ((x_T - m) * -0.5 * (v + eps)**-1.5 * dx_norm).sum(axis=0)
    dm = (dx_norm * -1 * (v + eps)**-0.5).sum(axis=0) + (dv * (x_T - m) * -2 / N).sum(axis=0)
    dx_T = dx_norm / (v + eps)**0.5 + dv * 2 * (x_T - m) / N + dm / N
    dx = dx_T.T
